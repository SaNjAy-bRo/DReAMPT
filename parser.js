...
function extractBetween(str, startRegex, endRegex) {
    const s = str.match(startRegex);
    if (!s) return null;
    const startIndex = s.index + s[0].length;
    const subset = str.substring(startIndex);
    const e = subset.match(endRegex);
    if (!e) return subset;
    return subset.substring(0, e.index).trim();
}
...
for (let item of mapping) {
    ...
    while ((kbMatch = kbRegex.exec(html)) !== null) {
        keyBenefits.push({
            ...
            title: kbMatch[2].replace(/<[^>]+>/g, '').trim(),
            desc: kbMatch[3].replace(/<[^>]+>/g, '').trim()
        });
    }

    // 3. Extract Features (Remaining content)
    // Looking for <h3> followed by <p> often near images. Let's just grab all <h3> followed by <p> that are not benefits.
    let features = [];
    const featRegex = /<img[^>]*src="([^"]+)"[^>]*>.*?<h3>(.*?)<\/h3>\s*<p>(.*?)<\/p>/gs;
    let fMatch;
    while ((fMatch = featRegex.exec(html)) !== null) {
        let img = fMatch[1].replace('../', '/').replace('collateral/', 'images/');
        let title = fMatch[2].replace(/<[^>]+>/g, '').trim();
        if (!keyBenefits.find(b => b.title === title)) { // Make sure we don't duplicate
            let desc = fMatch[3].replace(/<[^>]+>/g, '').trim();
            features.push({ img, title, desc });
        }
    }

    // Since we don't have grouped categories natively, we will split the features into two halves:
    // First half -> "Dedicated Tools" (grid layout)
    // Second half -> "Advanced Capabilities" (zig zag layout)
...
const KeyBenefits = ${JSON.stringify(keyBenefits, null, 2)};
const GridFeatures = ${JSON.stringify(gridFeatures, null, 2)};
const Capabilities = ${JSON.stringify(zigZagFeatures, null, 2)};
    ...
    <main className="pt-24 lg:pt-32 pb-0 bg-[#f9fafb]">
      ...
      <section className="relative bg-[#eceff3] pt-24 pb-20 lg:pt-32 lg:pb-32 border-b border-slate-200 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="${heroImg}" alt="Background" className="w-full h-full object-cover opacity-[0.25] object-center" />
          ...
          <div className="hidden lg:block w-full lg:w-[45%]">
            <Reveal delay={500} className="transform translate-x-4">
              <div className="rounded-2xl overflow-hidden shadow-[0_30px_60px_rgb(0,0,0,0.2)] border-[8px] border-white/90 bg-white">
                <img src="${heroImg}" alt="Interface" className="w-full h-auto aspect-[4/3] object-cover" />
              </div>
            </Reveal>
          </div>
        </div>
