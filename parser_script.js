...
for (let item of mapping) {
    ...
    while ((fMatch = featRegex.exec(html)) !== null) {
        ...
        if (!keyBenefits.find(b => b.title === title)) {
    ...
    compStr += "            <Reveal><h1 className=\"font-heading text-4xl font-extrabold leading-tight text-white drop-shadow-lg sm:text-5xl lg:text-6xl tracking-tight\">" + h1.replace(/"/g, "'") + "</h1></Reveal>\n";
    compStr += "            <Reveal delay={100}><div className=\"w-20 h-1.5 bg-brand-orange mt-6 rounded-full shadow-sm\"></div></Reveal>\n";
    compStr += "            <Reveal delay={200}><h2 className=\"mt-8 text-2xl font-bold text-white drop-shadow-md lg:text-3xl\">" + subtitle.replace(/"/g, "'") + "</h2></Reveal>\n";
    compStr += "            <Reveal delay={300}><p className=\"mt-5 text-lg text-zinc-100 drop-shadow max-w-2xl font-medium leading-relaxed\">" + description.replace(/"/g, "'") + "</p></Reveal>\n";
